...
class Control:
    ...
    def evaluate_signals(self, reactor, t):
        ...
        # evaluate signals
        # + psi_ytchen: self.input is a dictionary, self.input['signal'] is a list of dictionaries representing signals
        for s in self.input['signal']: 
            ...
            # psi_ytchen: mdot: pipe mass flowrate
            elif s['type'] == 'mdot':
                id = s['value'][0]  # pipe name
                if 'fluid' in reactor.solve and id in reactor.fluid.pipeid:
                    indx = reactor.fluid.pipeid.index(id)
                    mfr = (reactor.fluid.Gf[indx][0])*(reactor.fluid.areaz[indx])
                    self.signal[s['id']] = mfr # pipe mass flow rate
                else:
                ...
                if 'fluid' in reactor.solve and id in reactor.fluid.pipeid:
                    ...
                    if len(s['value']) == 1:
                        ...
                        for i in range(reactor.fluid.pipennodes[indx]):
                            ...
                            pro = reactor.data.matpro( {'type':reactor.fluid.type[indx], 'h':reactor.fluid.h[indx][i], 'p':reactor.fluid.p[indx][i],'ini':0} )   # psi_ytchen: adding Key 'ini'
                            davg += pro['rhof']
                        ...
                        if s['value'][1] > reactor.fluid.pipennodes[indx]:
                            ...
                            sys.exit()
                        # call material property function
                        # psi_ytchen: adding Key 'ini'
                        pro = reactor.data.matpro( {'type':reactor.fluid.type[indx], 'h':reactor.fluid.h[indx][int(s['value'][1])-1],'p':reactor.fluid.p[indx][int(s['value'][1])-1],'ini':0} )
                        self.signal[s['id']] = pro['rhof']
                else:
                    print('****ERROR: \'signal\' card ' + s['id'] + ' refers to pipe that does not exist or there is no \'solve fluid\' card.')
                    sys.exit()
            ...
            # if: conditional if
                ...
                self.signal[s['id']] = self.signal[s['value'][2]]  # psi_ytchen: false condition
            ...
            # lookup: table
                ...
                # scipy function
                f = interp1d(x, y)
                id = s['value'][0] # psi_ytchen: the string of x-variable name
        ...
        if 'fluid' in reactor.solve:
            for i in range(reactor.fluid.npipe):
                ...
                if reactor.fluid.pipetype[i] == 'normal' and reactor.fluid.signaltemp[i] != '':
                    ...
                    # + psi_ytchen: impose the corresponding h from temperature
                    pro = reactor.data.matpro( {'type':reactor.fluid.type[i], 't':reactor.fluid.temp[i],'p':reactor.fluid.p[i],'ini':1} )
                    reactor.fluid.h[i] = pro['h']
                    # - psi_ytchen
        # + psi_ytchen: check POWTAB input or not
        if 'powertable' in reactor.solve:
            if 'POWTAB' not in reactor.control.signal:
                print('***ERROR: there is no POWTAB in the signal input for powertable.')
                sys.exit()
            # - psi_ytchen:
    #----------------------------------------------------------------------------------------------
        ...
        for line in lines:
            ...
            if len(word) > 0:
                ...
                # thermal-hydraulic pipe with free level
                    ...
                    inp['pipe'].append( {'id':word[1], 'type':'freelevel', 'matid':word[2], 'dhyd':word[3], 'len':word[4], 'dir':0, 'areaz':word[5], 'nnodes':1, 'signaltemp':word[6]} )
                #--------------------------------------------------------------------------------------
                ...
                # models to be solved
                    ...
                    correct_values = {'fluid','fuelrod','htstr','pointkinetics','eigenvalueproblem','powertable'}
                    value = set([word[1]])
    ...
    def open_output_files(self, reactor):
        ...
        if 'fluid' in reactor.solve:
            ...
            for i in range(reactor.fluid.npipe):
                ...
                fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join([str(j).zfill(4).ljust(13) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                fid.append(open(path4results + os.sep + 'fluid-xe-' + reactor.fluid.pipeid[i] + '.dat', 'w'))
                fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join([str(j).zfill(4).ljust(13) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                fid.append(open(path4results + os.sep + 'fluid-alpha-' + reactor.fluid.pipeid[i] + '.dat', 'w'))
                fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join([str(j).zfill(4).ljust(13) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
    ...
    #----------------------------------------------------------------------------------------------
    def print_output_files(self, reactor, fid, time, dt):

        print('time(s): ' + '{0:12.5e} '.format(time) + ' dt(s):'+'{0:12.8e} '.format(dt))
        # print output files
        indx = 0
        if 'signal' in self.input:
            # signals
        ...
        if 'fluid' in reactor.solve:
            ...
            for i in range(reactor.fluid.npipe):
                ...
                indx += 1
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(['{0:12.5e} '.format(reactor.fluid.xe[i][j]) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                indx += 1
    ...
    def write_to_y(self, reactor):
        ...
        if 'fluid' in reactor.solve:
            ...
            for i in range(reactor.fluid.npipe):
                    # enthalpy in pipe nodes, psi_ytchen, use 'extend' function
                    y.extend( reactor.fluid.h[i] )
        if 'fuelrod' in reactor.solve:
            for i in range(reactor.solid.nfuelrods):
                for j in range(reactor.solid.fuelrod[i].nz):
        ...
        if 'fluid' in reactor.solve:
            ...
            for i in range(reactor.fluid.npipe):
                for j in range(reactor.fluid.pipennodes[i]):
                    # enthalpy in pipe nodes, psi_ytchen
                    reactor.fluid.h[i][j] = y[indx]
                    indx += 1
        if 'fuelrod' in reactor.solve:
            for i in range(reactor.solid.nfuelrods):
                for j in range(reactor.solid.fuelrod[i].nz):
